...
## ADR-065 — Google share-link resolution: encoded data + page fallback
...
Normal Google Maps mobile shares (`maps.app.goo.gl/…`) resolved to final
place URLs with no `@lat,lng` pin — only encoded `!3dLAT!4dLNG` place data
...
- Provider dispatch: `detectMapProvider()` → `resolveGoogleMaps()` /
